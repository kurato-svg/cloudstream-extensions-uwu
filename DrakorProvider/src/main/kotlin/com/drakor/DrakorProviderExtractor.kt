...
import com.drakor.DrakorProvider.Companion.idlixAPI
import com.drakor.DrakorProvider.Companion.RiveStreamAPI
...
object DrakorProviderExtractor : DrakorProvider() {
    ...
    suspend fun invokeVidsrccc(
        ...
        val url = if (season == null) {
            ...
            "${DrakorProvider.vidsrcccAPI}/v2/embed/tv/$tmdbId/$season/$episode"
        }
        ...
        val serverUrl = if (season == null) {
            "${DrakorProvider.vidsrcccAPI}/api/$tmdbId/servers?id=$tmdbId&type=movie&v=$v&vrf=$vrf&imdbId=$imdbId"
        } else {
                ...
                it.name.equals("VidPlay") -> {
                    ...
                    callback.invoke(
                        newExtractorLink(
                            ...
                            this.referer = "${DrakorProvider.vidsrcccAPI}/"
                        }
                    )
    ...
    suspend fun invokeVidsrc(
        ...
        val url = if (season == null) {
            "${DrakorProvider.vidSrcAPI}/embed/movie?imdb=$imdbId"
        } else {
            "${DrakorProvider.vidSrcAPI}/embed/tv?imdb=$imdbId&season=$season&episode=$episode"
        }
    ...
    suspend fun invokeXprime(
        ...
        runAllAsync(
            {
                val url = if (season == null) {
                    "${DrakorProvider.xprimeAPI}/${servers.first()}?id=$tmdbId"
                } else {
                    "${DrakorProvider.xprimeAPI}/${servers.first()}?id=$tmdbId&season=$season&episode=$episode"
                }
            ...
            {
                val url = if (season == null) {
                    "${DrakorProvider.xprimeAPI}/${servers.last()}?name=$title&fallback_year=$year"
                } else {
                    "${DrakorProvider.xprimeAPI}/${servers.last()}?name=$title&fallback_year=$year&season=$season&episode=$episode"
                }
    ...
    suspend fun invokeWatchsomuch(
        ...
        val epsId = app.post(
            "${DrakorProvider.watchSomuchAPI}/Watch/ajMovieTorrents.aspx", data = mapOf(
                "index" to "0",
                "mid" to "$id",
        ...
        val subUrl = if (season == null) {
            "${DrakorProvider.watchSomuchAPI}/Watch/ajMovieSubtitles.aspx?mid=$id&tid=$epsId&part="
        } else {
    ...
    suspend fun invokeMapple(
        ...
        val url = if (season == null) {
            "${DrakorProvider.mappleAPI}/watch/$mediaType/$tmdbId"
        } else {
            "${DrakorProvider.mappleAPI}/watch/$mediaType/$season-$episode/$tmdbId"
        }
        ...
        callback.invoke(
            newExtractorLink(
                ...
                this.referer = "${DrakorProvider.mappleAPI}/"
                this.headers = mapOf(
                    "Accept" to "*/*"
                )
            }
        ...
        tryParseJson<ArrayList<MappleSubtitle>>(subRes)?.map { subtitle ->
            subtitleCallback.invoke(
                newSubtitleFile(
                    subtitle.display ?: "",
                    fixUrl(subtitle.url ?: return@map, DrakorProvider.mappleAPI)
                )
            )
        }
    }
    ...
    suspend fun invokeVidlink(
        ...
        val url = if (season == null) {
            "${DrakorProvider.vidlinkAPI}/$type/$tmdbId"
        } else {
            "${DrakorProvider.vidlinkAPI}/$type/$tmdbId/$season/$episode"
        }
        ...
        callback.invoke(
            newExtractorLink(
                ...
                this.referer = "${DrakorProvider.vidlinkAPI}/"
            }
        )
    }
    ...
    suspend fun invokeVidfast(
        ...
        val type = if (season == null) "movie" else "tv"
        val url = if (season == null) {
            "${DrakorProvider.vidfastAPI}/$type/$tmdbId"
        } else {
            "${DrakorProvider.vidfastAPI}/$type/$tmdbId/$season/$episode"
        }

        val res = app.get(
            url, interceptor = WebViewResolver(
                Regex("""${DrakorProvider.vidfastAPI}/$module/JEwECseLZdY"""),
                timeout = 15_000L
            )
        ).text
    ...
    suspend fun invokeWyzie(
        ...
        val url = if (season == null) {
            "${DrakorProvider.wyzieAPI}/search?id=$tmdbId"
        } else {
            "${DrakorProvider.wyzieAPI}/search?id=$tmdbId&season=$season&episode=$episode"
        }
    ...
    suspend fun invokeVixsrc(
        ...
        val type = if (season == null) "movie" else "tv"
        val url = if (season == null) {
            "${DrakorProvider.vixsrcAPI}/$type/$tmdbId"
        } else {
            "${DrakorProvider.vixsrcAPI}/$type/$tmdbId/$season/$episode"
        }
        ...
        listOf(
            ...
            VixsrcSource("Vixsrc [Beta]", video2, "${DrakorProvider.mappleAPI}/"),
        ).map {
            callback.invoke(
                newExtractorLink(
                    it.name,
    ...
    suspend fun invokeSuperembed(
        ...
        val token = app.get("${DrakorProvider.superembedAPI}/directstream.php?video_id=$tmdbId&tmdb=1$path").url.substringAfter(
            "?play="
        )
    ...
    suspend fun invokeVidrock(
        ...
        val type = if (season == null) "movie" else "tv"
        val url = "${DrakorProvider.vidrockAPI}/$type/$tmdbId${if (type == "movie") "" else "/$season/$episode"}"
        val encryptData = VidrockHelper.encrypt(tmdbId, type, season, episode)

        app.get("${DrakorProvider.vidrockAPI}/api/$type/$encryptData", referer = url).parsedSafe<LinkedHashMap<String, HashMap<String, String>>>()
            ?.map { source ->
                if (source.key == "source2") {
                    ...
                    tryParseJson<ArrayList<VidrockSource>>(json)?.reversed()?.map mirror@{ it ->
                        callback.invoke(
                            newExtractorLink(
                                ...
                                this.headers = mapOf(
                                    "Range" to "bytes=0-",
                                    "Referer" to "${DrakorProvider.vidrockAPI}/"
                                )
                            }
                        )
                    }
                    ...
                    callback.invoke(
                        newExtractorLink(
                            ...
                            this.referer = "${DrakorProvider.vidrockAPI}/"
                            this.headers = mapOf(
                                "Origin" to DrakorProvider.vidrockAPI
                            )
                        }
                    )
                }
